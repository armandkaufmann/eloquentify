...
describe("Aggregates: Max", () => {
   test("It builds aggregate query without specified column", () => {
    ...
    test("It supports raw queries", () => {
        ...
        const countQueryResult = new Max(query, rawQuery).prepare();
        const expectedQuery = "SELECT MAX(LENGTH(name)) AS aggregate FROM (SELECT * FROM `users` WHERE `id` > ?) AS temp_table";
        const expectedBindings = [5];
