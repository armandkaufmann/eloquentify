...
describe("Aggregates: Sum", () => {
   test("It builds aggregate query without specified column", () => {
    ...
    test("It supports raw queries", () => {
        ...
        const countQueryResult = new Sum(query, rawQuery).prepare();
        const expectedQuery = "SELECT SUM(LENGTH(name)) AS aggregate FROM (SELECT * FROM `users` WHERE `id` > ?) AS temp_table";
        const expectedBindings = [5];
